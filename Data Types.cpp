...
/// program -> stmtseq
/// stmtseq -> stmt { ; stmt }
/// stmt -> ifstmt | repeatstmt | assignstmt | readstmt | writestmt | typestmt
/// ifstmt -> if exp then stmtseq [ else stmtseq ] end
/// repeatstmt -> repeat stmtseq until expr
/// assignstmt -> identifier := expr
/// readstmt -> read identifier
/// writestmt -> write expr
/// typestmt -> (int|bool|real) identifier
...
enum NodeKind{
                IF_NODE, REPEAT_NODE, ASSIGN_NODE, READ_NODE, WRITE_NODE,TYPES_INT_NODE, TYPES_BOOL_NODE,TYPES_REAL_NODE, /// في علاقه بين ال Types_Node and Int في نفس الترتيب
...
const char* NodeKindStr[]=
            {
                "If", "Repeat", "Assign", "Read", "Write", "Int","Bool", "Real",
                "Oper", "Num", "ID"
            };
...
/// typestmt -> (int|real|bool) identifier
TreeNode* TypeStmt(CompilerInfo* pci, ParseInfo* ppi)
{
    pci->debug_file.Out("Start TypesStmt");
    TreeNode* tree=new TreeNode;
    if(ppi->next_token.type== INT)
    {
        tree->node_kind=TYPES_INT_NODE;
        tree->line_num=pci->in_file.cur_line_num;
        Match(pci, ppi, INT);
    ...
    {
        tree->node_kind=TYPES_BOOL_NODE;
        tree->line_num=pci->in_file.cur_line_num;
        Match(pci, ppi, BOOL);
        if(ppi->next_token.type==ID) AllocateAndCopy(&tree->id, ppi->next_token.str);
        Match(pci, ppi, ID);
    ...
    {
        tree->node_kind=TYPES_REAL_NODE;
        tree->line_num=pci->in_file.cur_line_num;
        Match(pci, ppi, REAL);
        if(ppi->next_token.type==ID) AllocateAndCopy(&tree->id, ppi->next_token.str);
        Match(pci, ppi, ID);
    ...
    pci->debug_file.Out("End TypesStmt");
    return tree;
}
...
TreeNode* Stmt(CompilerInfo* pci, ParseInfo* ppi)
{
    ...
    else if(ppi->next_token.type==INT) tree=TypeStmt(pci, ppi);
    else if(ppi->next_token.type==BOOL) tree=TypeStmt(pci, ppi);
    else if(ppi->next_token.type==REAL) tree=TypeStmt(pci, ppi);
    else throw 0;
...
void PrintTree(TreeNode* node, int sh=0)
{
    ...
    else if(node->node_kind==ID_NODE || node->node_kind==READ_NODE || node->node_kind==ASSIGN_NODE ||node->node_kind == TYPES_INT_NODE  ||node->node_kind == TYPES_BOOL_NODE ||node->node_kind == TYPES_REAL_NODE ) printf("[%s]", node->id);
