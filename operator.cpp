...
/// term -> term2 { & term2 }         left associative
/// term2 -> factor { (*|/) factor }    left associative
...
/// term2 -> factor { (*|/) factor }    left associative
TreeNode* Term2(CompilerInfo* pci, ParseInfo* ppi)
{
...
TreeNode* Term(CompilerInfo* pci, ParseInfo* ppi)
{
    ...
    while( ppi->next_token.type==AND )
    {
        ...
        new_tree->child[0]=tree;
        Match(pci, ppi, ppi->next_token.type);
        new_tree->child[1]=Term2(pci, ppi);
...
int Evaluate(TreeNode* node, SymbolTable* symbol_table, int* variables)
{
    ...
    if(node->oper==AND)return (a>b) ? a-b : b-a ;
